...
import { LineChart, Line, Bar, BarChart, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Treemap, PieChart, Pie } from 'recharts';
...
class KivaChart extends Component {
	...
	render() {
			...
			return (
			<div>
			<BarChart width={600} height={300} data={this.state.data} margin={{top: 20, right: 10, left: 5, bottom: 5}}>
			...
			if (this.props.visualType == "pie") {
				...
				<Pie data={this.state.data} cx={200} cy={200} outerRadius={60} fill="#8884d8"/>
				<Pie data={this.state.data} cx={200} cy={200} innerRadius={70} outerRadius={90} fill="#82ca9d" label/>
	     	    </PieChart>
    			</div>
				)
			}

...
class KivaChart extends Component {
  ...
  render() {
    if (this.props.id == "Payment Chart") {
      ...
      for (i = 0; i < 13; i++) {
	this.state.repaymentData[i] = {periodNum: i+1, principal: this.props.data[4][i], interest: this.props.data[6][i], taxes: this.props.data[7][i], insurance: this.props.data[8][i], fees: this.props.data[9][i], securityDeposit: this.props.data[10][i]}; 
      }
      ...
      for (i = 0; i < 13; i++) {
        this.state.repaymentData[i] = {periodNum: i+1, balance: this.props.data[5][i]}; 
      }
    }
	...
	<Tooltip/>
	<Legend />
	<Bar dataKey={this.state.principalBalance} stackId="a" fill="#c94473" />
	...
	<Tooltip/>
	<Legend />
	<Line type="monotone" dataKey="interest" stroke="#c94473" activeDot={{r: 8}}/>    
	<Line type="monotone" dataKey={this.state.principalBalance} stroke="#a044c9" />
